...
def is_green_phase_state(state):
    """Green service phase: has green movements and is not a yellow transition."""
    return ("G" in state or "g" in state) and "y" not in state and "Y" not in state


def is_yellow_phase_state(state):
    """Yellow transition phase. Some right-on-red links may still show G/g."""
...
class DensityController:
    ...
    def __init__(self, tls_id):
        ...
        self.green_phases = [
            ...
            if is_green_phase_state(p.state)
        ]
        if not self.green_phases:
            raise ValueError("TLS %s khong co pha xanh hop le" % tls_id)

        self.yellow_after_green = self._build_yellow_map()
        ...
        log("  [TLS %s] %d pha | Webster S=%d PCE/h L=%ds" % (
            tls_id, len(self.green_phases), SATURATION_FLOW, LOST_TIME))
        for gp in self.green_phases:
            log("    Pha %d: %s" % (gp, self.phase_edges[gp]))
        for gp in self.green_phases:
            yp = self.yellow_after_green.get(gp)
            if yp is None:
                log("    Pha %d: khong tim thay pha vang, se chuyen xanh truc tiep" % gp)
            else:
                log("    Pha %d -> vang %d" % (gp, yp))
    ...
    def _build_yellow_map(self):
        """Map each green phase to the first following yellow before the next green."""
        yellow_map = {}
        green_set = set(self.green_phases)

        for gp in self.green_phases:
            yellow_idx = None
            for offset in range(1, self.num_phases + 1):
                idx = (gp + offset) % self.num_phases
                if idx in green_set:
                    break
                if is_yellow_phase_state(self.all_phases[idx].state):
                    yellow_idx = idx
                    break
            yellow_map[gp] = yellow_idx

        return yellow_map

    def _start_next_green(self, sim_time):
        ...
        log("  [GREEN] %s P%d XANH %.0fs (Webster)" % (
    ...
    def _shorten_current_green(self, sim_time, safe_remaining, reason):
        ...
        log("  [SHORTEN %s] P%d %.0fs -> %.0fs | remaining=%.0fs | %s" % (
            self.tls_id,
            self._cur_green(),
            old_target,
            new_target,
    ...
    def step(self, sim_time, is_gridlock, safe_shorten_seconds=SAFE_SHORTEN_SECONDS):
        ...
        if self.state == STATE_YELLOW:
            if sim_time - self.yellow_start >= YELLOW_TIME:
                self._start_next_green(sim_time)
            return
        ...
        # ── Bat den vang 3 giay ───────────────────────────────────────────────
        if should_switch:
            yellow_idx = self.yellow_after_green.get(self._cur_green())
            if yellow_idx is None:
                self.switch_count += 1
                log("  [WARN] %s P%d khong co pha vang | %s" % (
                    self.tls_id, self._cur_green(), reason))
                self._start_next_green(sim_time)
                return
            ...
            log("  [YELLOW] %s VANG 3s | %s" % (self.tls_id, reason))
            return
...
def run(args):
    global traci, LOG_ENABLED

    setup_traci_import()
    import traci as traci_module
    traci = traci_module
    LOG_ENABLED = args.realtime
    ...
    if not LOG_ENABLED:
        print("[FAST] --no-realtime: tat log pha va dashboard de chay nhanh hon")
    ...
    try:
        while traci.simulation.getMinExpectedNumber() > 0:
            ...
            if LOG_ENABLED and sim_time - last_dash >= DASHBOARD_INTERVAL:
