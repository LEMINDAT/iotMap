...
def get_options(args=None):
    ...
    op.add_argument("--max-turns", dest="max_turns", default=None, type=int,
                    help="Maximum number of turns allowed in the trip route")
...
class RandomTripGenerator:
    ...
    def count_turns(self, path):
        ...
        for i in range(1, len(path)):
            curr_edge = path[i]
            prev_edge = path[i-1]
            # Get direction from prev_edge to curr_edge
            from_node = prev_edge.getToNode()
            to_node = curr_edge.getFromNode()
            if from_node == to_node:
                # Same node, calculate angle
                prev_angle = math.atan2(prev_edge.getToNode().getCoord()[1] - prev_edge.getFromNode().getCoord()[1],
                                        prev_edge.getToNode().getCoord()[0] - prev_edge.getFromNode().getCoord()[0])
                curr_angle = math.atan2(curr_edge.getToNode().getCoord()[1] - curr_edge.getFromNode().getCoord()[1],
                                        curr_edge.getToNode().getCoord()[0] - curr_edge.getFromNode().getCoord()[0])
                angle_diff = minAngleDegreeDiff(prev_angle * 180 / math.pi, curr_angle * 180 / math.pi)
                if abs(angle_diff) > 10:  # threshold for turn
                    turns += 1
        return turns

    def get_trip(self, min_distance, max_distance, maxtries=100, junctionTaz=False, min_dist_fringe=None, max_turns=None, net=None):
        for min_dist in [min_distance, min_dist_fringe]:
            ...
            for _ in range(maxtries):
                ...
                if (distance >= min_dist
                    ...
                    # Check max_turns
                    if max_turns is not None and net is not None:
                        path = net.getShortestPath(source_edge, sink_edge)[0]
                        if path:
                            turns = self.count_turns(path)
                            if turns > max_turns:
                                continue
...
def createTrips(options, trip_generator, rerunFactor=None, skipValidation=False):
    ...
    def generate_origin_destination(trip_generator, options):
        source_edge, sink_edge, intermediate = trip_generator.get_trip(
            ...
            options.junctionTaz, options.min_dist_fringe, options.max_turns, options.net)
        return source_edge, sink_edge, intermediate
